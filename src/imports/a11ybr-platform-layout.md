Crie o layout de uma plataforma web chamada a11yBR, um hub colaborativo brasileiro focado em acessibilidade digital.

O objetivo do sistema é permitir que a comunidade envie conteúdos e perfis profissionais relacionados à acessibilidade digital. A interface deve ser limpa, editorial e focada em leitura e contribuição da comunidade.

Use design moderno, acessível e com forte hierarquia visual. Priorize boa legibilidade, espaçamento confortável e navegação clara.

Cor principal
hsl(222 74% 21%)

Estilo visual
minimalista
profissional
inspirado em plataformas de conhecimento e comunidades técnicas
alto contraste
foco em acessibilidade (WCAG)

Tipografia
sans-serif moderna
boa legibilidade para leitura longa

Layout geral do site

Header
logo a11yBR
menu principal

Itens do menu
Conteúdos
Comunidade
Submeter
Sobre
Newsletter

Página inicial

Hero section
Título
Hub brasileiro de acessibilidade digital

Subtítulo
Conteúdos, profissionais e recursos sobre acessibilidade reunidos em um único lugar.

Botões
Explorar conteúdos
Submeter conteúdo

Seção de categorias de conteúdo

Artigos
conteúdos escritos com análise, opinião ou estudo de caso

Comunidades
espaços de networking e troca sobre acessibilidade

Cursos e materiais
formações estruturadas e trilhas educacionais

Eventos
conferências, workshops, meetups ou webinars

Ferramentas
recursos técnicos para auditoria e testes de acessibilidade

Multimídia
podcasts e canais de vídeo

Sites e sistemas
produtos digitais com foco em acessibilidade

Mostrar como cards com ícones.

Seção comunidade

Rede de profissionais e organizações que atuam com acessibilidade.

Categorias de perfil

Profissionais de tecnologia
designers, desenvolvedores, QA, product managers

Empresas e ONGs

Intérpretes de Libras

Audiodescritores

Tradutores de Braille

Página de submissão

Essa página deve dividir claramente duas ações principais.

Bloco 1
Submeter conteúdo

Descrição
Envie artigos, ferramentas, eventos, cursos ou outros recursos sobre acessibilidade digital.

Botão
Submeter conteúdo

Bloco 2
Submeter perfil profissional

Descrição
Cadastre seu perfil ou organização para fazer parte da comunidade a11yBR.

Botão
Criar perfil

Cada bloco deve ter ícone, título e descrição.

Formulário de submissão de conteúdo

Campos

Tipo de conteúdo
select com opções
artigo
comunidade
curso
evento
ferramenta
multimídia
site ou sistema

Título

Descrição

Link

Autor ou organização

Imagem opcional

Botão enviar

Página sobre

Seções

Missão
Explicar que o a11yBR é um hub colaborativo de acessibilidade digital focado no contexto brasileiro.

Como funciona

Submissão comunitária
qualquer pessoa pode enviar conteúdo

Revisão editorial
conteúdos são analisados

Publicação aprovada
conteúdos aprovados são publicados

Atualização contínua
novos conteúdos adicionados semanalmente

Equipe
quem mantém o projeto

Código de conduta

Regras da comunidade

respeito
linguagem inclusiva
reconhecimento do trabalho de outras pessoas
reportar comportamentos inadequados

Declaração de acessibilidade

Informar que o site busca cumprir WCAG 2.1 nível AA.

Newsletter

Seção simples de inscrição

Título
Receba novos conteúdos sobre acessibilidade

Campo email

Botão inscrever

Rodapé

links

Sobre
Submeter conteúdo
Comunidade
Código de conduta
Declaração de acessibilidade

Contato
acessibilidade@a11ybr.com.br

Extras de UX importantes

usar HTML semântico
contraste alto
foco visível
labels claros em formulários
layout responsivo
componentes acessíveis
